...
namespace RuleEngine.Evidence
{
    ...
    public class ExpressionEvaluator
    {
        ...
        private static readonly string LogicalRegEx = @"((?<!\()\)|\((?!\))|>=|<=|!=|==|<|>|AND|OR|NOT|ISNULL|&&|\|\||XOR|\x2b|\x2d|\x2a|\x2f)";//增加了(?<!\()\)|\((?!\))使其支持方法
        ...
        private bool IsFact(string s)
        {
            ...
            {
                if (Char.IsLetter(c) || Char.IsNumber(c)||c=='_'||c=='$' )//add _ and $ by studyzy
                    continue;
        ...
        public Symbol Evaluate()
        {
            ...
            foreach (Symbol s in postfix)
            {
                ...
                else if (s.type == Type.Fact)
                {
                    ...
                    op3.value = new Naked(fact.Value, 
                        fact.ValueType, 
                        s.expressionStr);
        ...
        private Symbol EvaluateGreaterThan(Symbol op1, Symbol op2)
        {
            ...
            try
            {
                o1 = Convert.ToDecimal(op1.value.Value);
                o2 = Convert.ToDecimal(op2.value.Value);
