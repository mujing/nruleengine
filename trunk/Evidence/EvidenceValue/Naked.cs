...
namespace RuleEngine.Evidence.EvidenceValue
{
    ...
    public class Naked : IEvidenceValue
    {
        ...
        private bool isFunction(string ex)
        {
            if (ex.EndsWith("()"))
            {
                return true;
        ...
        public Naked(object value, Type valueType, string expressionStr)
        {
            this.expressionStr = expressionStr;

            if (expressionStr != null &&
                expressionStr.Trim() != string.Empty)
            {
                string[] exArray = expressionStr.Split('.');

                for (int i = 1; i < exArray.Length; i++)
                {
                    string exStr = exArray[i];

                    if (isFunction(exStr))
                    {
                        exStr = exStr.Substring(0, exStr.IndexOf("("));

                        MethodInfo mi  = 
                            valueType.GetMethod(exStr);
                    ...
                    else
                    {
                        ...
                        {
                            FieldInfo fInfo = valueType.GetField(exStr);
